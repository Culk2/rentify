...
app.get('/make-server-0d8d1b3e/users/:id', async (c) => {
  try {
    const user = await getUserFromToken(c.req.raw);
    if (!user) {
      return c.json({ error: 'Unauthorized' }, 401);
...
app.get('/make-server-0d8d1b3e/items', async (c) => {
  try {
    ...
    items = items.filter((item: any) => item != null && typeof item === 'object');
...
app.post('/make-server-0d8d1b3e/items/:id/upload-image', async (c) => {
  try {
    ...
    return c.json({ success: true, imageUrl: urlData.signedUrl, item: updatedItem });
  } catch (error) {
...
app.post('/make-server-0d8d1b3e/seed-demo-data', async (c) => {
  try {
    console.log('Starting demo data seeding...');

    // Create demo user if not exists
    const demoUserId = 'demo-user-id-12345';
    let demoUser = await kv.get(`user:${demoUserId}`);
    
    if (!demoUser) {
      demoUser = {
        id: demoUserId,
        email: 'demo@rantify.com',
        name: 'Demo Owner',
        avatar: 'https://ui-avatars.com/api/?name=Demo+Owner&size=200',
        createdAt: new Date().toISOString(),
      };
    ...
    const sampleItems = [
      {
        ...
        description: 'Canon EOS 5D Mark IV with 24-70mm lens. Perfect for events and portraits.',
        price: 45,
        priceUnit: 'day',
      ...
      {
        title: 'Mountain Bike',
        description: 'Full suspension mountain bike, ideal for trails and rough terrain.',
        price: 25,
        priceUnit: 'day',
        category: 'Sports',
        location: 'Maribor',
      ...
      },
      {
        ...
        description: 'Spacious 4-person tent, waterproof and easy to set up.',
        price: 20,
        priceUnit: 'day',
        category: 'Camping',
        location: 'Kranj',
      ...
      },
      {
        ...
        description: 'Single person kayak, perfect for lakes and calm rivers.',
        price: 30,
        priceUnit: 'day',
      ...
      },
      {
        ...
        description: 'PS5 with 2 controllers and popular games included.',
        price: 35,
        priceUnit: 'day',
        category: 'Gaming',
        location: 'Ljubljana',
      ...
      },
      {
        ...
        description: 'Fender Stratocaster with Marshall amplifier. Great for practice or gigs.',
        price: 40,
        priceUnit: 'day',
        category: 'Music',
        location: 'Ljubljana',
      ...
      },
      {
        ...
        description: 'DJI Mavic Pro with 4K camera, perfect for aerial photography.',
        price: 50,
        priceUnit: 'day',
      ...
      },
      {
        ...
        description: 'Large capacity backpack with rain cover, perfect for multi-day hikes.',
        price: 12,
        priceUnit: 'day',
        category: 'Outdoor',
        location: 'Kranjska Gora',
        imageUrl: 'https://images.unsplash.com/photo-1622260614927-5c0139044fb4?w=800&h=600&fit=crop',
      },
      {
        title: 'GoPro Hero 11',
        description: 'Latest GoPro action camera with accessories and mounts.',
        price: 30,
        priceUnit: 'day',
        category: 'Photography',
        location: 'Novo Mesto',
    ...
    await kv.set(`listings:user:${demoUserId}`, userListings);
    console.log(`Seed complete! Created ${createdCount} demo items`);

    return c.json({
      success: true,
      message: `Successfully seeded ${createdCount} demo items`,
